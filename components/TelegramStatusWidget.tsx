...
export const TelegramStatusWidget: React.FC<TelegramStatusWidgetProps> = ({ config, loading, onUpdateConfig, token }) => {
  ...
  if (loading) {
    return (
      <div className="h-24 animate-pulse rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3">
  ...
  return (
    <div className="shadow-lg rounded-2xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 p-3 transition-all">
       {!isLinked ? (
          <div className="flex items-center justify-between gap-3">
             <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                    <span className="text-xs font-black text-gray-800 dark:text-gray-200">Telegram</span>
             ...
             <Button 
                ...
                className="w-auto px-4 py-1.5 h-8 text-[10px] font-bold uppercase"
             >
                Connect
             </Button>
          </div>
          ...
          <div className="flex flex-col gap-3">
             ...
             <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 min-w-0">
                    {/* Icon */}
                    <div className="w-9 h-9 flex-shrink-0 rounded-full bg-[#24A1DE]/10 text-[#24A1DE] flex items-center justify-center">
                    ...
                    <div className="flex flex-col min-w-0">
                        <div className="flex items-center gap-1.5">
                            <span className="text-sm font-black text-gray-800 dark:text-white truncate">
                                {config?.telegramUsername ? `@${config.telegramUsername}` : 'Linked'}
                            </span>
                            <span className="px-1 py-0.5 rounded bg-emerald-500/10 border border-emerald-500/20 text-[8px] font-black text-emerald-600 uppercase tracking-tight">
                                Linked
                            </span>
                        </div>
                        {config?.linkedAt && (
             ...
             <div className="grid grid-cols-3 gap-1 p-1 bg-gray-100 dark:bg-gray-800/50 rounded-lg border border-gray-200 dark:border-gray-700">
                  <button
                    ...
                    disabled={isUpdating}
                    className={`py-1.5 rounded-md text-[9px] font-bold uppercase transition-all flex items-center justify-center gap-1 ${
                      visibility === 'public' 
